...
class jssdk_native_video():

    def __init__(self,count=0):
        self.count=count
        self.sum_num=0
        p = ParseConfigfile()
        self.url = p.getOptionValue('url', 'nv_url')
        self.new_url,self.params=params2dict(self.url)
        self.app_id=p.getOptionValue('native_video','app_id')
        self.unit_id=p.getOptionValue('native_video', 'unit_id')
        self.sign=p.getOptionValue('native_video', 'sign')
        self.content_type=p.getOptionValue('native_video','content_type')
